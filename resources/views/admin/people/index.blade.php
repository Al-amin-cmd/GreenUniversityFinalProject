<x-admin.master>
    ...
    <div class="headmenu position-relative bg-light bg-gradient rounded pt-2">
        <h2 class="h2 text-info">Refugee Deatils List</h2>
        <div class="btn-toolbar mb-2 mb-md-0 position-absolute top-0 end-0 pt-1 ">
            <div class="btn-group me-2">

                @can('superAdmin')
                    <a href="{{ route('people.trash') }}"><button type="button"
            ...
            <tbody>
                @foreach ($peoples as $row)
                    <tr>
                        ...
                        <td style="width: 90px">{{ $row->nid_code }}</td>
                        ...
                        <td><button type="button" class="btn btn-warning"><a
                                    href="{{ route('people.show', $row->id) }}"
                                    class="text-white text-decoration-none">Id Card</a></button>
                            <button type="button" class="btn btn-primary tex"><a
                                    href="{{ route('people.edit', $row->id) }}"
